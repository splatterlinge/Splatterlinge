...
const AObject * Splatterbug::intersectLine( const AObject * exclude, const QVector3D & origin, const QVector3D & direction, float & length, QVector3D * normal )
{
   const AObject * nearestTarget = AObject::intersectLine( exclude, origin, direction, length, normal );
	float rayLength;
...
void Splatterbug::updateSelf( const double & delta )
{
	switch( state() )
	{
		...
		case ALIVE:
		{
			...
			qDebug() << "player detected: " <<playerDetected;
			if( dist < 10 )
			{
...
//                        }
			...
			else if( playerDetected )
			{
				...
				float worldPos =world()->landscape()->terrain()->getHeight(newPos)+15;
				float curWorldPos  =worldPosition().y();
				if(worldPos < curWorldPos){
